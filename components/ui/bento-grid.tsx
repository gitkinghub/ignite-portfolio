...
import { GlobeDemo } from "./GridGlobe"; //check for import here
...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText('housescreen1@gmail.com');
  ...
  return (
    ...
    >
      ...
      {/* the section under landing page(after hero section) */}
      <div className={`${id===6 && 'flex justify-center'} h-full `}>
        {/* the image to display in the second section under hero */}
        ...
        <div className={cn(titleClassName, 'group-hover/bento:translate-x-2 transition duration-200 relative md:h-full min-h-40 flex flex-col px-5 p-5 lg:p-10')} >
          ...
          {id === 3 && (
            <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2" >
              ...
              <div className="flex flex-col gap-3 md:gap-3 lg:gap-8">
                <span className="py-4 px-3 rounded-lg text-center bg-[#10132e]"/>
                {['ExpressJs', 'MongoDB', 'Javascript'].map((item)=>(
                  <span key={item} className="py-2 lg:py4 lg:px-3 px-3 text-xs lg:text-base opacity-50  lg:opacity-100 rounded-lg text-center bg-[#10132E]">
                    {item}
                  </span>
                ))}
